...
{
    public class DotNetManagerJob : IJob
    {
        private IAgentDotNetMetric _repository;
        // private readonly ILogger _logger;
        public DotNetManagerJob(IAgentDotNetMetric repository)
        {
            // _logger = logger;
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            foreach (var agent in agents)
            {
                ...
                var metrics = response.GetAllDotNetMetrics(request);
                if (metrics != null)
                {
